...
sudo osm --hostname 192.168.100.24 vim-create --name osm-group13 --user osm --password osm --tenant OSM --auth_url $OS_AUTH_URL --account_type openstack --config '{security_groups: default, keypair: osm-kp}'
sudo osm --hostname 192.168.100.24 vim-create --name osm-group13 --user osm --password osm --tenant OSM --auth_url $OS_AUTH_URL --account_type openstack --config '{security_groups: default, use_floating_ip: True, keypair: osm-kp}'


mac1=$(echo tag1|md5sum|sed 's/^\(..\)\(..\)\(..\)\(..\)\(..\).*$/02:\1:\2:\3:\4:\5/')
mac2=$(echo tag2|md5sum|sed 's/^\(..\)\(..\)\(..\)\(..\)\(..\).*$/02:\1:\2:\3:\4:\5/')
mac3=$(echo tag3|md5sum|sed 's/^\(..\)\(..\)\(..\)\(..\)\(..\).*$/02:\1:\2:\3:\4:\5/')
mac4=$(echo tag4|md5sum|sed 's/^\(..\)\(..\)\(..\)\(..\)\(..\).*$/02:\1:\2:\3:\4:\5/')
