...
class webLoadBalancer(app_manager.RyuApp):
    ...
    def switch2_features_handler(self, ev):
        ...
        for port in [4,5,6,7]:
            # In_port=1 -> Pop tag valan and route it to table_id=vlan
            match = parser.OFPMatch(in_port=1,vlan_vid=(0x1000 | self.port_to_tag[port]))
            ...
            # On table=vlan_id, set out_port = 3
            match = parser.OFPMatch(in_port=1)
            actions = [parser.OFPActionOutput(3)]
            self.add_flow(datapath, 1000,  match=match, actions=actions, table_id=self.port_to_tag[port], instructions=None)

            # On table_vlan_id, match ARP && ARP requests and send it to controller (filtered)
            match = parser.OFPMatch(in_port=1, eth_type=ether_types.ETH_TYPE_ARP, arp_op=arp.ARP_REQUEST)
            actions = [parser.OFPActionOutput(ofproto.OFPP_CONTROLLER,ofproto.OFPCML_NO_BUFFER)]
            self.add_flow(datapath, 900,  match=match, actions=actions, table_id=self.port_to_tag[port], instructions=None)

            # On table=0, match ARP && ARP reply coming from port=3 and send it to controller
            match = parser.OFPMatch(in_port=3, eth_type=ether_types.ETH_TYPE_ARP, arp_op=arp.ARP_REPLY)
            actions = [parser.OFPActionOutput(ofproto.OFPP_CONTROLLER,ofproto.OFPCML_NO_BUFFER)]
            self.add_flow(datapath, 900,  match=match, actions=actions, table_id=0, instructions=None)
        ...
        buckets = []
        for tag in [231,232,233,234]:
            actions = [parser.OFPActionPushVlan(ether_types.ETH_TYPE_8021Q), parser.OFPActionSetField(vlan_vid=(0x1000 | tag)), parser.OFPActionOutput(1)]
        ...
        self.add_flow(datapath, 950, match=match, actions=actions, table_id=0)
